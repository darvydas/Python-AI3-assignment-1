...
def display_book(book:Book, due_date = None, reader_id = ''):
  ...
  if reader_id and due_date:
    print(f"| {reader_id:<10} | {str(due_date):<10} | {book.title:<30} | {book.author:<15} | {book.publication_year:<5} | {book.genre:<10} | {book.available:<5} | ")
  else:
    print(f"| {book.title:<30} | {book.author:<15} | {book.publication_year:<5} | {book.genre:<10} | {book.available:<5} |")

def display_all_books(books):
  """Displays details of all books in the library."""
  if books:
    print("-"*81)
    print(f"| {'Title:':<30} | {'Author:':<15} | {'Year:':<5} | {'Genre':<10} | {'Stock':<5} |")
    print("-"*81)
    for book in books:
      display_book(book)
    print("-"*81)
...
def display_overdue_books(overdue_books):
  ...
  if overdue_books:
    ...
    print("Overdue books:")
    print("-"*107)
    print(f"| {'Reader ID:':<10} | {'Due date:':<10} | {'Title:':<30} | {'Author:':<15} | {'Year':<5} | {'Genre':<10} | {'Stock':<5} |")
    print("-"*107)
